...
class DatasetCLM:
    ...
    def conform_filling(chunk):
        names = chunk['a_concepts']
        relationship = chunk['relation']
        subjects = chunk['b_concepts']

        # generate samples
        texts_generated, masks = [], []
        for name in names:
            for subject in subjects:
                text_generated = f'{name} {relationship} '
                mask = subject
    ...
    def generate_texts(starting_text, part_a, relationship, part_b, masks, how):
        ...
        if how == 'left':
            for element, _mask in zip(part_a, masks):
                texts_generated.append((f'{starting_text}{element} {relationship} {random.choice(part_b)}.', _mask))
        elif how == 'right':
            for element, _mask in zip(part_b, masks):
                texts_generated.append((f'{starting_text}{random.choice(part_a)} {relationship} {element}.', _mask))
